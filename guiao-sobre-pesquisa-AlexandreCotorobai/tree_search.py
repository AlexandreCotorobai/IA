...
class SearchNode:
    def __init__(self,state,parent,depth,cost = 0, heuristic = 0, action = None): 
        self.state = state
        self.parent = parent
        self.depth = depth
        self.cost = cost
    ...
    def in_parent(self, newstate):
        if self.parent is None:
            return False
        elif self.parent.state == newstate:
            return True
        else:
...
class SearchTree:
    ...
    def average_depth(self):
        return self.total_depth / (self.non_terminals + self.terminals)
